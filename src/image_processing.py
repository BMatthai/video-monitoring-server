...
def shape_detection(cascade, frame, color):
...
def detect_shapes(frame):
	if (available_cooldown(TimeManager.last_detect_time, 1) == True):
		...
		frame = shape_detection(cars_cascade, frame, GREEN_COLOR)
		frame = shape_detection(fullbody_cascade, frame, RED_COLOR)
		if (VideoRecorder.recording == False):
			start_recording()
	return True, frame
